...
# A description must state its trigger condition, or the agent cannot route on it.
USE_PHRASES = ("use when", "use at", "use before", "use after", "use during")
...
def validate(skill_dir: Path) -> list[str]:
    ...
    if not isinstance(desc, str) or not desc.strip():
        ...
        lowered = desc.lower()
        if not any(p in lowered for p in USE_PHRASES):
            fail(f"description should say when to use the skill (expected one of: {sorted(USE_PHRASES)})")
