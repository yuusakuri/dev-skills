...
description: 'ソフトウェア開発の作業を始めるとき、あるいは今どの工程にいるのか、どのスキルが当てはまるのか分からないときに使います。作業を該当する工程へ割り当て、その工程を担うコミュニティのスキルを名指しします。要件定義、設計、計画、実装、デバッグ、検証、レビュー、リリース、本番運用が対象です。'
license: MIT
metadata:
...
このスキルは地図であって、方法ではありません。固有の実践を持ちません。

仕事は、作業がどの工程にあるかを判断し、その工程を扱う既存のコミュニティのスキルへ引き渡すことです。いずれも各自のリポジトリで保守されています。

表で名指ししたスキルを起動してください。ここで工程の作業そのものをしようとしないでください。
...
そのうえで該当する行を選びます。作業が複数の工程にまたがるなら、順に実行してください。前の工程を飛ばすことが、手戻りの最大の原因です。

## 工程の対応表

スキル名は`plugin:skill`の形で示します。未導入のスキルでも、行を見れば何を探せばよいかは分かります。リポジトリと導入コマンドは`docs/reference/catalog.md`にあります。
...
以下はこの集まりの運用原則です。詳細は個々のスキルが持ちます。

1. 主張の前に根拠を出します。
   それを示す出力なしに、工程の完了を報告しないでください。「テストが通る」とは、実行して結果を読んだという意味です。
2. 可逆性が厳密さを決めます。
   安く元に戻せるなら、そのまま進めます。高くつく、または一方通行なら、先に決定を書き残します（`architecture-decision-record`）。
3. 工程1にコードはありません。
   「これは何をすべきか」に答えるためにコードを書いているなら、工程を間違えています。使い捨てと明示した検証は別です。
4. 範囲を決めるのは依頼者です。
   抜けや危険は示しますが、頼まれた範囲を黙って広げたり狭めたりしないでください。
5. 記録を残します。
   要件、決定、障害はリポジトリに置きます。失われる会話の中だけに置かないでください。
...
| 大きさ | 例 | 通す工程 |
| --- | --- | --- |
| ごく小さい | 誤字、コメント、明らかな1行修正 | 4から6。仕様書もADRも要らない |
| 小さい | 影響が閉じている、既存の型に沿う、元に戻せる | 1、4、6、7 |
| 標準 | 新しい機能やエンドポイント | 0、1、2、3、4、6、7、8 |
| 影響が大きい | データモデル、認証、金銭、マイグレーション、公開API | 全工程。`threat-model`と`architecture-decision-record`は必須 |

どの大きさか判断がつかないときは、大きい方とみなし、そう述べてください。

## 把握しておくべき空白

すべての工程が、保守されているスキルへ割り当たっています。ただし、真正面から扱うスキルがない箇所が2つ残っています。

テストのないレガシーコードが1つ目です。

`code-simplification`は挙動を保ち、リファクタリングを機能変更のコミットから分離しますが、挙動が変わったことを何かが知らせてくれる前提に立っています。これから形を変えようとしているコードを固定するテストがない場合は、まず特性化テストを書いてください。そのコードが現在何をしているかを、不具合ごと写し取るテストです。そのうえでリファクタリングします。

タイミング以外の原因による不安定なテストが2つ目です。

`superpowers:systematic-debugging`に同梱されている`condition-based-waiting.md`は、最大の原因を扱います。`sleep`や`setTimeout`でタイミングを当て推量するために、手元では通るのに負荷時やCIで落ちるテストです。まずこれを使ってください。

ただし、テストの実行順と共有状態、乱数の種の未固定、順序のないコレクションの比較、手元とCIの環境差は扱いません。これらについては、不安定さは実在する非決定性であるという規則に従い、原因を突き止めてください。再実行で緑にしないでください。

`constraint-driven-development`は、この失敗のもう半分を捉える対になるスキルです。エージェントが緑にするためにテストを飛ばしたり消したりする場合を扱います。

どの行にも当てはまらない場合は、その作業がどの工程にあるかを述べ、スキルを当て推量せず、上記の工程の原則に従って進めてください。
