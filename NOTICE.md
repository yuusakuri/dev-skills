...
referenced `SKILL.md` from its pinned commit and checks that every documented install
command names a marketplace and plugin that really exist.
...
that has vendored a copy. Vendored copies drift: at the time of writing, one widely used
mirror of `addyosmani/agent-skills` is pinned several revisions behind upstream, and its
copy of `security-and-hardening` is missing roughly 57 lines that exist upstream.
...
Superpowers skills cross-reference each other as `superpowers:<skill-name>`, so they are
installed under the plugin name `superpowers` and those references resolve.

## Deliberate exclusions

- **Anthropic's document skills** (`docx`, `pdf`, `pptx`, `xlsx`) are *source-available,
  not open source* — "(c) 2025 Anthropic, PBC. All rights reserved." Install them from
  Anthropic's own marketplace and accept their terms if you want them.
- **`doc-coauthoring`** (`anthropics/skills`) ships without a licence file.
- **`rohitg00/awesome-claude-code-toolkit`** (2.6k stars, Apache-2.0) was curated and
  then removed: its marketplace exposes only command-based plugins, and the repository's
  own `skills/` directory is not an installable plugin, so the one entry taken from it
  could never be installed by the documented method.
- **`phuryn/pm-skills`** (26.3k stars, MIT): `pre-mortem` (4.1 KB) and `retro` (2.8 KB)
  add little over `ship-gate` and `launch-readiness`.
...
Upstream skill *names* and plugin names are part of this repository's routing and install
instructions, so a rename upstream fails verification by design.
